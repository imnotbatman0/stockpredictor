# Updated layers.py with GRU+Attention and Temporal Attention Pooling
...
class GRUWithAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = F.softmax(self.attn(out), dim=1)  # [batch, seq, 1]
        weighted = out * weights  # [batch, seq, hidden]
...
class TemporalAttentionPooling(nn.Module):
    ...
    def forward(self, x):  # x: [batch, seq, dim]
        ...
        attn_weights = F.softmax(self.v(u), dim=1)  # [batch, seq, 1]
        out = (attn_weights * x).sum(dim=1)  # [batch, dim]
        return out
